...
DirectionalLightShadows::DirectionalLightShadows(const char * const pWindowName, const unsigned int width, const unsigned int height)
	: Engine(pWindowName, width, height)
{
}


DirectionalLightShadows::~DirectionalLightShadows()
{
	delete m_VBSphere;
	delete m_IBSphere;
...
bool DirectionalLightShadows::StartUp()
{
	Engine::StartUp();
	...
	// ==================
	// Directional light
	m_TestDir.m_LightType = LightType::Directional;
	m_TestDir.m_Color = BVFloat4(0.9f, 0.9f, 0.9f);
	VectorToFloat3(VectorNormalize(VectorSet(1.0f, -1.0f, 1.0f)), m_TestDir.m_Direction);
	m_TestDir.m_Intensity = 1.0f;
	m_TestDir.m_Enabled = true;
	m_TestDir.m_CastsShadow = true;

	// ==================
	// Shadow Data
	m_pShadowMap = new ShadowMap();
	m_pShadowMap->Create(4096, 1, 1, 1);
...
void DirectionalLightShadows::Update()
{
...
void DirectionalLightShadows::Render()
{
	...
	m_pShadowMap->SetDirShadowPass(m_Camera, m_TestDir, 0);

	m_pGraphics->SetRasterizerState(m_pDepthBiasRS);

	m_pShadowMap->SetDirShadowParams(mGrid);
	...
	{
		m_pShadowMap->SetDirShadowParams(mSphere[i]);
	...
	for (unsigned int i = 0; i < 4; i++)
	{
		m_pShadowMap->SetDirShadowParams(mBox[i]);
	...
	m_pShadowMap->UnsetDirShadowPass();
	...
	m_pGraphics->SetSamplers(ShaderType::Pixel, &m_pSamplerClamp, 1);

	Light pLights[] = { m_Hems, m_TestDir };
