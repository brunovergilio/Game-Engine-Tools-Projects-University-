...
AnimBlendShader::AnimBlendShader()
{
}
...
bool AnimBlendShader::Initialize(const Graphics * const pGraphics, const char * const pShaderName)
{
...
void AnimBlendShader::MapData(const Graphics * const pGraphics, const ShaderParameters * const pShaderParameters)
{
	AnimBlendShaderParameters *pParams = (AnimBlendShaderParameters *)pShaderParameters;
	...
	{
		Interpolate(pGraphics, ((AnimBlendShaderParameters *)pShaderParameters)->pInterpolationData);
	}
	else
	{
...
void AnimBlendShader::Interpolate(const Graphics * const pGraphics, const ShaderInterpolationData * const pShaderInterpolationData)
{
	ID3D11DeviceContext *pContext = pGraphics->GetContext();

	m_pCBInterpolationInfo->Map(pGraphics, &pShaderInterpolationData->lerp, sizeof(float) * 4);
	ID3D11Buffer *pBuffers[] = { m_pCBInterpolationInfo->Get(), pShaderInterpolationData->pBeginAnimBuffer, 
		pShaderInterpolationData->pEndAnimBuffer, pShaderInterpolationData->pBlendAnimBuffer, pShaderInterpolationData->pJointsBuffer };

	pContext->VSSetConstantBuffers(0, 5, pBuffers);
...
void AnimBlendShader::SetPerFrameData(PointLight pointLight, XMFLOAT3 viewPos)
{
